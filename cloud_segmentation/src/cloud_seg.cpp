...
public:
    CloudSegmentor() :
            running(false), updateImage(false), updateCloud(false), nh("~"), spinner(0), running_viewers(true)
    {
...
private:
    ...
    void start()
    {
        ...
        string clrimg_topic_name, cloud_output_topic_name;
        string bbtopic_name, sd_depth_topic_name, sd_camerainfo_topic_name;

        if(!nh.hasParam("input_topic_color"))
            ROS_ERROR("Missing \"input_topic_color\" ");
        if(!nh.hasParam("output_topic_cloud"))
            ROS_ERROR("Missing \"output_topic_cloud\" ");
        if(!nh.hasParam("input_bounding_boxes"))
            ROS_ERROR("Missing \"input_bounding_boxes\" ");
        if(!nh.hasParam("sd_depth_image"))
            ROS_ERROR("Missing \"sd_depth_image\" ");
        if(!nh.hasParam("sd_camerainfo"))
            ROS_ERROR("Missing \"sd_camerainfo\" ");

        nh.getParam("input_topic_color", clrimg_topic_name);
        nh.getParam("output_topic_cloud", cloud_output_topic_name);
        nh.getParam("input_bounding_boxes", bbtopic_name);
        nh.getParam("sd_depth_image", sd_depth_topic_name);
        nh.getParam("sd_camerainfo", sd_camerainfo_topic_name);

        ROS_INFO("%s\n%s", clrimg_topic_name.c_str(), cloud_output_topic_name.c_str());


        message_filters::Subscriber<Image> image_color_sub(nh, clrimg_topic_name, 1);
        message_filters::Subscriber<darknet_ros_msgs::BoundingBoxes> bounding_boxes (nh, bbtopic_name, 1);
        message_filters::Subscriber<Image> sd_depth_sub(nh, sd_depth_topic_name, 1);
        message_filters::Subscriber<CameraInfo> sd_cameraInfo_sub(nh, sd_camerainfo_topic_name, 1);
